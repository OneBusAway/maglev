...
func Prepare(ctx context.Context, db DBTX) (*Queries, error) {
	...
	if q.searchStopsStmt, err = db.PrepareContext(ctx, searchStops); err != nil {
		return nil, fmt.Errorf("error preparing query SearchStops: %w", err)
	}
...
func (q *Queries) Close() error {
	...
	if q.searchStopsStmt != nil {
		if cerr := q.searchStopsStmt.Close(); cerr != nil {
			err = fmt.Errorf("error closing searchStopsStmt: %w", cerr)
		}
	}
...
func (q *Queries) WithTx(tx *sql.Tx) *Queries {
	return &Queries{
		...
		searchStopsStmt:                           q.searchStopsStmt,
