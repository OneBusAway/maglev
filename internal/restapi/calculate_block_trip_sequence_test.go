...
func TestCalculateBlockTripSequence(t *testing.T) {
	...
	for _, trip := range trips {
		...
		if len(activeTripIDs) >= 2 {
			multiTripBlock = &blockInfo{tripIDs: activeTripIDs, totalTrips: len(blockTrips)}
			break
		}
	}
	...
	t.Run("sequence values are within block bounds", func(t *testing.T) {
		for _, tripID := range multiTripBlock.tripIDs {
			seq := api.calculateBlockTripSequence(ctx, tripID, serviceDate)
			assert.GreaterOrEqual(t, seq, 0)
			assert.Less(t, seq, multiTripBlock.totalTrips,
				"Sequence for trip %s should be within [0, %d), got %d", tripID, multiTripBlock.totalTrips, seq)
		}
	})

	t.Run("invalid trip ID returns -1", func(t *testing.T) {
		sequence := api.calculateBlockTripSequence(ctx, "invalid-trip-id", serviceDate)
		assert.Equal(t, -1, sequence)
	})

	t.Run("date outside service range returns -1", func(t *testing.T) {
