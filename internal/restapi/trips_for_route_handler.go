...
func (api *RestAPI) tripsForRouteHandler(w http.ResponseWriter, r *http.Request) {
	...
	if len(indexIDs) == 0 && len(layoverBlocks) == 0 {
		...
		response := models.NewListResponseWithRange([]models.TripsForRouteListEntry{}, references, false, false)
	...
	response := models.NewListResponseWithRange(result, references, false, false)
	api.sendResponse(w, r, response)
}
