...
func (api *RestAPI) tripsForRouteHandler(w http.ResponseWriter, r *http.Request) {
	...
	if len(indexIDs) > 0 {
		blocksFromIndices, err := api.GtfsManager.GtfsDB.Queries.GetBlocksForBlockTripIndexIDs(ctx, gtfsdb.GetBlocksForBlockTripIndexIDsParams{
			FromTime:   sql.NullInt64{Int64: timeRangeStart, Valid: true},
			ToTime:     sql.NullInt64{Int64: timeRangeEnd, Valid: true},
			IndexIds:   indexIDs,
			ServiceIds: serviceIDs,
		})
		if err != nil {
	...
	if len(prevServiceIDs) > 0 {
		...
		if err != nil {
			...
			prevFromTime := prevDayNanosSinceMidnight + timeRangeStart - currentNanosSinceMidnight
			prevToTime := prevDayNanosSinceMidnight + timeRangeEnd - currentNanosSinceMidnight
			prevBlocks, err := api.GtfsManager.GtfsDB.Queries.GetBlocksForBlockTripIndexIDs(ctx, gtfsdb.GetBlocksForBlockTripIndexIDsParams{
				FromTime:   sql.NullInt64{Int64: prevFromTime, Valid: true},
				ToTime:     sql.NullInt64{Int64: prevToTime, Valid: true},
