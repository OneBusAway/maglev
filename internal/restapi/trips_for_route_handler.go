...
func (api *RestAPI) tripsForRouteHandler(w http.ResponseWriter, r *http.Request) {
	...
	for _, vehicle := range duplicatedVehicles {
		...
		// GTFS-RT requires start_date on a DUPLICATED trip's descriptor — it's the only
		// authoritative service date for a trip that, by definition, may run at an
		// entirely different date/time than the base trip it copies. Legacy OBA
		// (DuplicatedTripServiceParserImpl) makes the same assumption: it always derives
		// the service date from the feed's start_date, never from the base trip's static
		// schedule.
